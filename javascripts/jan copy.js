...
function evaluateHand() {
	...
	//calculate sum of card values for each hand
	var mySum = 0;
	var yourSum = 0;
	for (i = 0; i <= 4; i++) {
		mySum = mySum + myCards[i].value;
		yourSum = yourSum + yourCards[i].value;
	}
	//
	//Make aces high if not needed for a low run - first for myCards
	//
	if (mySum !== 15) { 					//if not a low run...
	...
	//
	if (yourSum !== 15) { 									//if not a low run...
		for (i = 0; i <=4; i++) {							//loop through the hand
	...
	if (myPoints < 400) {											//don't try this if there is already a flush or straight
		...
		if ((myPairsTotal === 3) && (myPairsArray[0] === 0 || myPairsArray[3] === 0)) {	//full house
